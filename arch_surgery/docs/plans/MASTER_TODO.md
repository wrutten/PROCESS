...
| **A19** | **frozen-input-convergence** — tests the one assumption A2's STOP rests on. A2 measured `Sᵢ` *inside* the coupled loop, where a downstream module cannot settle while its inputs are still moving, so `S₂`/`S₃` are plausibly biased high and the estimate biased **against** the partition. Freeze M1's outputs, iterate M2 alone, count sweeps — measures `Sᵢ` under partitioning conditions **without building the partition**. Also tests the per-loop ordering `S₁ ≤ S₂ ≤ S₃`, which would show the near-equality is M1 gating rather than module dynamics | A2 ✓ | **IN PROGRESS** (dispatched 2026-08-31). Sensitivity: `low_aspect_ratio_DEMO`'s partition contribution is 7.2 % as measured but ≈27 % if the downstream modules reach the two-sweep floor — **the gate boundary sits inside the uncertainty on that scenario** |
...
1c. **Should A13 (feed-forward hoist) be revived?** The only positive-return intervention that is
   also *separable* — 4.6–8.2 % at `k = 0`, no new design variables, no dimension penalty.
   **User decision.**
