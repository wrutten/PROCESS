...
1. ~~Which module is the laggard?~~ **Answered by A2: none is** under the coupled loop. `S_global ≈
   S₁ ≈ S₂ ≈ S₃`; M1 joint-last in 82–85 % of loops and never strictly ahead — the case §3.2 named
   as fatal.
...
1. ~~**Which module is the laggard?**~~ **ANSWERED twice.** A2: none is, in the coupled loop.
   **A19: under partitioning it is M2** — joint-last in 73–76 % of the two large pulsed
   tokamaks' loops and strictly last in 36–39 %, because `S₂` is the one count frozen inputs do
   not change. M2 is 10 of the 46 module nodes but 42 % of the measured cost, so the plan's
   condition (a *small* module driving the loop) is still not met.
