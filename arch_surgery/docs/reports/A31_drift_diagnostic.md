...
| **Script** | [`arch_surgery/idf_probe/a31_drift_probe.py`](../../idf_probe/a31_drift_probe.py) — every number below comes from its stages (`neutrality`, `trace`, `retrace`, `a0p`, `analyze --with-a0p`); instrument committed at `a1a4a9ce`, the campaign-census extension of the analyze stage at `03ea8a8e`, and the full analyze stage re-executed at `03ea8a8e` on a clean tree with identical output (protocol §15). Source line numbers are verified against the frozen base commit with `git show c0ae5b28:…`, never the working tree (A27's lesson) |
