...
| 2026-09-02 | `gen_ystate.py --check` began reporting DIFFERS on all four A18 records, because A26 enriched the descriptive `census`. The **records were refreshed, not the check weakened** — `components_sha256`, which is what the predicate depends on and what `replay.py` aborts on, is byte-identical on all four decks, and no component record changed. The reproduction gate was then re-run **in full at the final commit** — all four decks, both hoist settings — and still reports 0 differing of 4 800 arm records over 64 800 record keys with 32/32 sensitivity cases caught, so §2's table is the state of the tree as merged, not of an earlier one. |

---

## 15. Where the artifacts are

Untracked, under `arch_surgery/idf_probe/runs/` (repository policy: summaries and verdicts are
committed, raw run JSONs are not):

| what | where |
|---|---|
| reproduction gate, both hoist settings | `runs/a26/_gate_hoist0.json`, `_gate_hoist1.json` |
| cost-versus-accuracy ladders, 17 rungs × 3 decks | `runs/a26/<deck>/replay_acc_*/result.json` |
| the matched-accuracy analysis | `runs/a26/matched_accuracy.json` |
| the A26 spec sweep, three floors | `runs/a26/<deck>/replay_spec_a26_floor*/result.json` |
| timings | `runs/a26/<deck>/replay_timing/result.json` |
| the `pulse` pre-predicate gate | `runs/a26_pulse/_pulse_gate.json` |

Tracked and committed: the A26 coupling-state records
`arch_surgery/docs/data/ystate_a26_<deck>.json` and the per-node write-set census
`arch_surgery/docs/data/node_writesets.json`.

Everything is regenerated by `python MDA_partition_experiment.py --stages method_gate accuracy
pulse_gate`, or by `arch_surgery/fixedpoint/run_a26.py` and
`arch_surgery/idf_probe/run_a26_pulse.py` directly.

**Harvest provenance.** All of it replays A18's recorded harvests, read through symlinks into the
main checkout's untracked run tree — the harvests are 35 MB each and are not duplicated per
worktree. The licence for that reuse is §2.1's reproduction gate, not §6.3's hash argument, which
no longer holds.
